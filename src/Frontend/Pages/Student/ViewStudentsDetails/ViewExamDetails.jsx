  ...
  const fetchResults = async (examType) => {
    ...
    try {
      const response = await axios.get(
        `${url}mark/students/${StudentData.StudentData.studentData?._id}/exams/${examType}`
      );
  ...
  const deleteResult = async (subjectId) => {
    try {
      ...
      const response = await axios.delete(
        `${url}mark/students/${StudentData.studentData?._id}/subject/${subjectId}`
      );
                  ...
                  {loading ? (
                    ...
                    results.map((result, index) => (
                      <tr key={index}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <button
                            onClick={() => deleteResult(result?.subjectId)}

...
const ViewStudentFees = (studentData) => {
  ...
  const id =
    studentData?.studentData?._id || studentData?.studentData?.student?._id;
  const studentName =
    studentData?.studentData?.name ||
    studentData?.studentData?.student?.name ||
    "Student";
  const studentClass =
    studentData?.studentData?.class?.name ||
    studentData?.studentData?.student?.class?.name ||
    "N/A";
  ...
  const totalPendingAmount = transactions
    .filter((t) => t.status !== "paid")
    .reduce((sum, t) => sum + (t.baseAmount || 0), 0);
  const pendingFine = transactions
    .filter((t) => t.paidFine !== "paid")
    .reduce((sum, t) => sum + (t.lateFineAmount || 0), 0);
  const totalLateFineAmount = transactions.reduce(
    (sum, t) => sum + (t.lateFineAmount || 0),
    0
  );
  const totalAmount = transactions.reduce(
    (sum, t) => sum + (t.baseAmount || 0),
    0
  );
  ...
  return (
    <div className="sm:px-16 px-6 sm:py-8 py-6">
      ...
      <div className="bg-white p-4 rounded-md mb-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          <div className="p-3 border rounded-md">
            <p className="text-black-200 text-sm">Student Name</p>
            <p className="font-semibold text-black-300">{studentName}</p>
          </div>
          {/* <div className="p-3 border rounded-md">
            <p className="text-black-200 text-sm">Class</p>
            <p className="font-semibold text-black-300">{studentClass}</p>
          </div> */}
          <div className="p-3 border rounded-md">
            <p className="text-black-200 text-sm">Total Base Amount</p>
            <p className="font-semibold text-black-300">₹{totalAmount}</p>
          </div>
          <div className="p-3 border rounded-md md:col-span-1">
            <p className="text-black-200 text-sm">Total Late Fine</p>
            <p className="font-semibold text-amber-600">
              ₹{totalLateFineAmount}
            </p>
          </div>
          <div className="p-3 border rounded-md md:col-span-1">
            <p className="text-black-200 text-sm">Total Payable Amount</p>
            <p className="font-semibold text-blue-600">
              ₹{totalAmount + totalLateFineAmount}
            </p>
          </div>

          <div className="p-3 border rounded-md">
            <p className="text-black-200 text-sm">Total Paid</p>
            <p className="font-semibold text-green-600">
              ₹{totalPaidAmount + pendingFine}
            </p>
          </div>
          <div className="p-3 border rounded-md">
            <p className="text-black-200 text-sm">Total Pending</p>
      ...
      <div className="bg-white p-2 rounded-md">
        ...
        />
        {error && (
          <div className="p-2 mb-4 text-black-200 rounded">{error}</div>
        )}

        {transactions.length === 0 && (
          <div className="flex justify-center items-center p-4 text-black-200">
